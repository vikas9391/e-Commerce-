...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <div className="bg-white rounded-xl overflow-hidden transition-all duration-300 shadow-md hover:shadow-xl border border-gray-100 hover:border-medical-primary/30 group">
      <Link to={`/products/${product.slug}`}>
        ...
        <div className="relative w-full pt-[100%] overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100">
          ...
          {product.category?.name === 'Prescription' && (
            <div className="absolute top-3 left-3 bg-medical-primary text-white px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wide shadow-lg flex items-center gap-1.5">
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="p-5">
          ...
          {/* Product Name */}
          <h3 className="text-lg font-bold mb-3 text-gray-800 line-clamp-2 leading-tight min-h-[3.5rem] group-hover:text-medical-primary transition-colors">
            {product.name}
          </h3>
          ...
          <div className="flex justify-between items-center pt-4 border-t border-gray-100">
            ...
            <div className="flex items-baseline gap-1">
              ...
              <span className="text-2xl font-bold text-medical-primary">{product.price}</span>
            </div>
          ...
          {/* View Details Button */}
          <button className="mt-4 w-full py-2.5 bg-gradient-to-r from-medical-primary to-medical-secondary text-white rounded-lg font-semibold text-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 flex items-center justify-center gap-2 group-hover:gap-3">
            View Details
