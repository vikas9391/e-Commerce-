...
const Navbar = () => {
  ...
  const handlePrescriptionClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    if (!isAuthenticated) {
      e.preventDefault();
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 bg-white shadow-md z-50 transition-all duration-300">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16 sm:h-20">
        {/* Logo */}
        <Link to="/" className="flex items-center gap-2 sm:gap-3 text-dark text-xl sm:text-2xl font-heading font-bold transition-all duration-300 hover:opacity-80 z-10" onClick={closeMobileMenu}>
          <div className="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-br from-blue-600 to-cyan-600 rounded-lg flex items-center justify-center shadow-md flex-shrink-0">
        ...
        <div className="hidden lg:flex items-center gap-8">
          ...
          <Link 
            ...
            className="text-gray-700 font-semibold text-[15px] relative group flex items-center gap-2 transition-colors duration-300 hover:text-blue-600"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex lg:hidden items-center gap-3 z-10">
          {/* Cart Icon (Mobile) */}
          <Link to="/cart" className="relative text-gray-700 transition-all duration-300 p-2 rounded-lg hover:bg-gray-100 hover:text-blue-600" onClick={closeMobileMenu}>
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
            </svg>
            {items.length > 0 && (
              <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center shadow-md">
                {items.length}
              </span>
            )}
          </Link>
          ...
          <button
            ...
            className="hamburger-button text-gray-700 p-2 rounded-lg hover:bg-gray-100 transition-colors"
      ...
      {/* Mobile Menu - COMPLETELY REDESIGNED FOR NO OVERFLOW */}
      <div 
      ...
      >
        ...
        {/* Menu Panel - Full width, no side margins */}
        <div 
          ref={mobileMenuRef}
          className={`absolute top-16 sm:top-20 left-0 right-0 bottom-0 bg-white overflow-y-auto transition-transform duration-300 ${
          ...
          <div className="w-full p-4 sm:p-6">
            ...
            {isAuthenticated && (
              <div className="mb-4 sm:mb-6 pb-4 sm:pb-6 border-b border-gray-200">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-gradient-to-br from-blue-600 to-cyan-600 rounded-full flex items-center justify-center text-white font-bold text-lg flex-shrink-0">
                    {user?.username.charAt(0).toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-gray-900 truncate text-sm sm:text-base">{user?.username}</p>
                    {isAdmin && (
                      <span className="inline-block mt-1 px-2 py-0.5 text-xs font-bold bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-full">
                        Admin
                      </span>
                    )}
                  </div>
            ...
            <nav className="space-y-1">
              <Link
                to="/"
                onClick={closeMobileMenu}
                className="flex items-center gap-3 px-3 py-2.5 text-gray-700 font-semibold rounded-lg hover:bg-blue-50 hover:text-blue-600 transition-colors text-sm sm:text-base"
              >
              ...
              {isAuthenticated ? (
                <>
                  ...
                  <Link
                    to="/profile"
                    onClick={closeMobileMenu}
                    className="flex items-center gap-3 px-3 py-2.5 text-gray-700 font-semibold rounded-lg hover:bg-gray-50 transition-colors text-sm sm:text-base"
                  >
                    <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                    </svg>
                  ...
                  <button
                    onClick={handleLogout}
                    className="flex items-center gap-3 px-3 py-2.5 text-red-600 font-semibold rounded-lg hover:bg-red-50 transition-colors w-full text-sm sm:text-base"
                  >
                    <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                    </svg>
                ...
                <Link
                  to="/login"
                  onClick={closeMobileMenu}
                  className="flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg bg-gradient-to-r from-blue-600 to-cyan-600 text-white font-heading font-semibold transition-all duration-300 hover:shadow-lg mt-3 text-sm sm:text-base"
                >
                  <svg className="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
